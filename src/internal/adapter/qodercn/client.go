...
// Spec 返回 QoderCN 能力声明。reasoning 与 tools 支持，SSE-only，支持签到。
...
func (a *Adapter) Kind() channel.Kind { return channel.QoderCN }

// Spec 实现 channel.Channel。
func (a *Adapter) Spec() channel.Spec { return Spec() }

// Login 实现 channel.Channel。QoderCN 是 OAuth 设备流，需要浏览器授权，
// 属于异步交互流程（M1 未接入控制台授权屏，见 M2）。这里返回明确错误，
// 不做静默成功 —— 红线一。
func (a *Adapter) Login(ctx context.Context) (*channel.Credential, error) {
	return nil, errs.New(errs.AuthFailed, "渠道授权是异步交互流程（浏览器设备流），请在面板「渠道授权」页完成；本方法不阻塞等待").WithChannel(string(channel.QoderCN))
}
...
func (a *Adapter) Chat(ctx context.Context, c *channel.Credential, req channel.ChatRequest) (channel.Stream, error) {
	...
	if resp.StatusCode >= 400 {
		...
		return nil, errs.New(kind, "上游返回错误").
			WithChannel(string(channel.QoderCN)).
			WithAccount(c.UID).
