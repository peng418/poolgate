...
// 代码级移植自 wild-work internal/qodercn/cosy.go（源自 qoder2api），
// 适配 PoolGate 的 channel.Credential 后逐行保留。
package qodercn
...
// machineFingerprint 是机器指纹的稳定派生（缺省值在 credential.Extra 里）。
...
func newCosySession(fp machineFingerprint, nickname, uid, dt, drt, userType string) (*CosySession, error) {
	if fp.MachineID == "" || fp.MachineToken == "" || fp.MachineType == "" {
