// login.go QoderCN 的面板授权：OAuth 设备流（PKCE + 轮询）。
//
// 协议移植自 wild-work internal/login_qodercn（已实测）：授权页只带
// nonce/challenge/client_id，用户点完后本服务轮询 deviceToken/poll，
// 上游用 HTTP 404/202 表示「还没授权」。
//
// 与 wild-work 的差别：状态不落盘，而是随 LoginSession 存在内存里 ——
// 授权是 5 分钟内的一次性交互，落盘只会留下需要清理的临时文件。
package qodercn
...
// 授权相关端点与常量。
const (
	OAuthWebsite = "https://qoder.com.cn"
