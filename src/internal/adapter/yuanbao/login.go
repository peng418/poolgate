...
// 我们认 x-uskey（cookie 顺带带上更接近真实客户端）。注意：两份参考实现在凭证形态上
// 并不一致 —— free-api 整段头重放（含 x-uskey），chat2api 只用 Cookie 的 hy_user/hy_token。
// x-uskey 是浏览器请求里真实存在的鉴权头，我们以它为准，但不宣称它是「唯一被校验的东西」。
//
