// login.go WorkBuddyCN 的面板授权：state 轮询式（非标准设备流）。
//
// 协议移植自 wild-work internal/login（已实测）：
//
...
//
// 与 wild-work 的差别：state 存在内存里（授权是 5 分钟的一次性交互），
// 且起始返回的 authUrl 会先跟随 301 解析成最终地址再给用户 —— 让用户少跳一次。
package workbuddy
...
func (a *Adapter) StartLogin(ctx context.Context, opts channel.LoginOptions) (channel.LoginSession, error) {
	...
	if err != nil {
		return nil, errs.New(errs.Transport, "构造授权请求失败").WithChannel(string(channel.WorkBuddyCN)).WithCause(err)
	}
	commonHeaders(req)
	...
	if err := json.Unmarshal(data, &st); err != nil || st.State == "" || st.AuthURL == "" {
		return nil, errs.New(errs.Parse, "上游未返回 state/authUrl，无法发起授权").
			WithChannel(string(channel.WorkBuddyCN)).WithUpstream(truncate(string(data), 200))
	}
