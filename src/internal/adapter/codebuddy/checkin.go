// checkin.go CodeBuddy 每日签到（与 WorkBuddy 共用同一套 billing 后端，按包内自持复制）。
//
//	POST /v2/billing/meter/checkin-activity-status  → {active, today_checked_in, daily_credit}
//	POST /v2/billing/meter/daily-checkin            → 领取
//
// 先探活动状态再决定要不要领：`active=false` 表示该账号/该渠道当前没有签到活动，
// 这时如实标注「无活动」，而不是报一个假的签到成功（红线一）。
// 已签到由 `today_checked_in` 判定，不靠错误码猜 —— 上游对「今天已签到」也会回业务码。
package codebuddy
	...
	if len(bytes.TrimSpace(raw)) > 0 {
		...
		if json.Unmarshal(raw, &resp) == nil && resp.Code != 0 {
			...
			return channel.CheckinResult{}, errs.New(errs.UpstreamFault, "签到领取失败").
				WithChannel(string(channel.CodeBuddy)).WithAccount(c.UID).
...
// billingPost 向 billing 端点发一个 JSON POST（带当前身份档的域名与 UA）。
...
// itoa 避免为一个小转换引入 strconv 依赖（与 workbuddy 同写法）。
