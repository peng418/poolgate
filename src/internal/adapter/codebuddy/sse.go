// sse.go CodeBuddy 的 SSE 处理：上游已是 OpenAI 兼容 SSE，逐行解析 data: 帧
// 归一成 channel.ChatCompletionChunk。
//
// 与 WorkBuddy 的实现同源（本包按「包内自持」原则复制过来，不改动 workbuddy 包）：
// delta 的 content / reasoning_content / tool_calls 三路都透传，usage 帧与
// finish_reason 都要原样交给客户端 —— 工具调用分片（name 与 arguments 分几帧到达）
// 靠 index 拼回，丢失任何一帧都会让客户端拼不出调用。
package codebuddy
