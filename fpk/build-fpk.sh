...
M="$PKG/manifest"
{
  ...
  printf 'changelog = 千问办公长对话修复（2026-09-27）。① 真机定案：上游 chat-ws 的单个消息上限是 32768 字节，超一字节就被 close 1009（message too big，read limited at 32769 bytes）立刻掐断；我们此前每一轮都把客户端给的全量历史塞进一个帧，上下文一长（实测 27k tokens）就必然越线，而客户端只看到「上游连接中断且未收到内容」——上游明明给了真原因，我们没说（破自己的红线一）。② 改为复用上游会话：把客户端对话映射到上游会话，每一轮只发最新一轮；重挂连接时带 join.lastSeqId 等于已消费到的最大事件序号（实测带 0 会触发上游全量 replay 并丢上下文），并整体屏蔽 replay_start 到 replay_end 之间的重放帧（否则上一轮的正文会漏进本轮，重放里的 idle 还会把本轮提前结束）。③ 发前按真实帧字节自查：超限在建会话之前就拒绝，不建会话、不消耗上游配额。④ 如实归类：超限与上游 close 1009 均归 PromptTooLong，并透传上游原话与上限数值，不再谎报「连接中断」，面板失败类型也有对应标签。⑤ 台账只在一轮成功后落库（失败即摘除），上限 8 段 LRU，45 分钟未用即过期。⑥ 系统提示有变化时补发一次；补发放不下就退回只发最新一轮并落日志，绝不静默截断上下文、也不把整轮打死。新增 5 条回归测试（复用只发最新一轮、超限在建会话前拒绝、close 1009 归类、replay 帧不得混入本轮、大历史加大系统提示仍可用），并已对真上游跑通多轮端到端。 | FPK %s\n' "$VERSION"
} >> "$M"
chmod 0644 "$M"
sed -n '1,20p' "$M" | sed 's/^/  /'
