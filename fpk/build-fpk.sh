...
M="$PKG/manifest"
{
  ...
  printf 'changelog = 六项冷却纪律修复（2026-09-27）。① 恢复参考实现的「连续错误门槛」：连接中断/空流这类本地推断错误先计数，连续 err_threshold 次（默认 5）才冷却，成功即清零，不再一错即冷。② 连接中断恢复计入账号错误 —— 0.9.8 的完全豁免让「一直在断的号」也记不下来。③ 单账号渠道纪律：池里只有一个可用账号时不冷却它，冷它等于整条渠道下线。④ 面板显示「连续错误 n/m」与「降级使用」，设置屏可调门槛与冷却时长。⑤ 成功即解冻：门槛冷却属软冷却，该号仍会被选用（排在健康号之后），一次成功（含体检探通）即自动解除，不再死等到时间结束；体检也能探到冷却中的号。⑥ 千问办公 409 CONCURRENT_OPERATION 不再当限流冷号 60 秒：真机实测「同一账号会话启动」闸门 2 秒级就放行（加 0.2 秒时 409、2.4 秒时 201），现在单列 SessionBusy、不冷却、适配器内退避重试（400ms 到 1.2s）并落日志；同账号会话令牌改为可被 ctx 取消，等不到就明确报账号忙，而不是把请求冻死在锁上。新增回归测试，顺带修好一条长期红的用例。 | FPK %s\n' "$VERSION"
} >> "$M"
chmod 0644 "$M"
sed -n '1,20p' "$M" | sed 's/^/  /'
