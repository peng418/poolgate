...
> **立项日期**：2026-09-24 ｜ **状态**：需求 / 设计方案已确认；实现到 **0.4.1**（安装包见 [Releases](https://github.com/peng418/poolgate/releases)）
...
**当前版本 0.3.7**（2026-09-25）：新增面板内改密、凭证自动续期（治「千问办公 token 过期后全 401」）、
修掉「上游 401 被当成管理员会话过期 → 点一下按钮就被踢回登录页」；0.3.6 加了「只下发可用模型」开关；
0.3.5 补上「对外基址一键复制」「面板内显示/生成网关 API Key」「新增账号自动拉余额」，并修掉「设置里的监听端口从未生效」等三处静默问题。
逐版本细节见 [`src/README.md`](src/README.md) 的 0.3.x 小节。
