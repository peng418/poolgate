...
**当前版本 0.4.1**（2026-09-25）：**授权不跳转** —— 面板内渲染能扫的**真二维码**（手机扫码完成登录，
桌面零跳转）、TraeWork 回调改用面板自己的局域网地址（去掉「必须与 PoolGate 同机」的限制）、
千问办公给「粘贴回调地址」兜底；点「授权」后**能出码就不弹浏览器**，只有出不了码的渠道才自动打开授权页。
之前几版：0.3.9 千问办公换成上游认的 device_token（治「重新登录也一样 401」）、0.3.7 面板内改密 + 凭证自动续期 +
